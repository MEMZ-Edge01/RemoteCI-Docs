...
云端中转默认开启，关闭开关只在单独的“RemoteCI 开发者设置”页显示。关闭后插件不会再向 WebUI 同步状态、账号和课表，通常只应用于调试局域网直连。
...
云端连接默认开启，普通“连接”页不提供关闭开关。需要测试纯局域网模式时，可在“设置 → 开发者”关闭；密码登录仍会临时使用云端完成认证，避免退出账号后无法恢复登录。
