...
- 手表：连接 WebUI 后，在“设置 → 更新”选择正式版或 Beta 渠道；可强制重新下载并覆盖同一版本，但不能降级。更新器只选择协议主版本相同的 v3.x.x Release，不再要求目标软件版本低于 WebUI；通过系统安装器覆盖安装时仍要求发布包与当前安装包签名一致。
...
升级前应先备份数据库卷。三端软件小版本可以不同，但必须保持协议号为 3；不支持的新功能由能力协商隐藏，破坏性协议变更必须升级到 V4 并协调升级。
