...
RemoteCI 以飞牛 fnOS 应用（`.fpk`）形式发布，采用 Docker 应用形态：容器内运行 RemoteCI WebUI，数据保存在 fnOS 数据卷，可由应用中心统一管理启动、停止和升级。稳定版本使用四段纯数字标签（当前为 `3.2.1.0`），Beta 使用 `v3.x.x-beta.y` 且不进入插件市场；旧 `v3.2.0` 安装的首次迁移需要手动安装新 FPK。
