...
WebUI 登录后进入"个人账号 → 系统更新"（管理员可见）。在飞牛 fnOS 环境下该面板会：

1. 自动检查 GitHub 最新 release 并显示版本与更新说明。
2. 发现新版本后一键把 `RemoteCI-<版本>.fpk` 下载到数据卷，并提供"保存到本机"链接。
3. 在飞牛应用中心选择该 fpk 手动安装，应用中心会自动拉取新版本镜像并重建容器，完成升级。

::: warning 安装确认必须手动
fnOS 目前没有开放"容器内应用自我安装/升级"的接口，因此最后一步"在应用中心确认安装"无法由 WebUI 代劳。应用上架飞牛应用中心后，用户可以直接走应用中心的常规升级通道。
:::
